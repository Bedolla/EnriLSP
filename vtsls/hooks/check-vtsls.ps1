...
<#
.SYNOPSIS
    EnriLSP - VTSLS (TypeScript/JavaScript) Language Server installer
.DESCRIPTION
    Checks for vtsls installation and auto-installs Node.js runtime and vtsls if missing.
...
class VtslsInstaller {
  ...
  VtslsInstaller() {
    $this.EnvManager = [EnvironmentManager]::new("vtsls")
  ...
  [bool] InstallLsp() {
    $this.EnvManager.WriteInfo("Installing vtsls and TypeScript...")
    ...
    $this.PkgInstaller.InstallWithNpm("@vtsls/language-server")
    $this.PkgInstaller.InstallWithNpm("typescript")

    if ($this.IsLspInstalled()) {
      $this.EnvManager.WriteSuccess("vtsls installed successfully")
      return $true
    }

    $this.EnvManager.WriteError("Failed to install vtsls. Please run manually:")
    $this.EnvManager.WriteError("  npm install -g @vtsls/language-server typescript")
    return $false
  }
  ...
  [int] Run() {
    ...
    if ($this.IsLspInstalled()) {
      ...
      $this.AddLspToPath()
      $this.EnvManager.WriteSuccess("vtsls is already installed")
      return 0
    }
...
[VtslsInstaller] $installer = [VtslsInstaller]::new()
exit $installer.Run()

...
<#
.SYNOPSIS
    EnriLSP - TailwindCSS Language Server installer
.DESCRIPTION
    Checks for @tailwindcss/language-server installation and auto-installs via npm.
    Provides TailwindCSS IntelliSense support.
.NOTES
    Author: Bedolla
    License: MIT
#>
...
class TailwindCssLspInstaller {
  hidden [EnvironmentManager] $EnvManager
  hidden [PackageInstaller] $PkgInstaller
  hidden [string] $NpmBinPath = "$env:APPDATA\npm"

  TailwindCssLspInstaller() {
    $this.EnvManager = [EnvironmentManager]::new("tailwindcss-lsp")
  ...
  [bool] IsLspInstalled() {
    ...
    return $this.EnvManager.CommandExists("tailwindcss-language-server")
  }
  ...
  [bool] InstallLsp() {
    $this.EnvManager.WriteInfo("Installing @tailwindcss/language-server via npm...")
    
    try {
      $process = Start-Process -FilePath "npm" -ArgumentList "install", "-g", "@tailwindcss/language-server" -Wait -PassThru -NoNewWindow
      if ($process.ExitCode -eq 0) {
        $this.EnvManager.AddToUserPath($this.NpmBinPath)
        $this.EnvManager.RefreshSessionPath()
        $this.EnvManager.WriteSuccess("@tailwindcss/language-server installed successfully")
        return $true
      }
    }
    catch {
  ...
  [int] Run() {
    if ($this.IsLspInstalled()) {
      $this.EnvManager.WriteSuccess("tailwindcss-language-server is already installed")
      return 0
    }
    ...
    if (-not $this.InstallLsp()) {
      ...
      $this.EnvManager.WriteError("  npm install -g @tailwindcss/language-server")
    }
    return 0
  }
}
...
[TailwindCssLspInstaller] $installer = [TailwindCssLspInstaller]::new()
exit $installer.Run()

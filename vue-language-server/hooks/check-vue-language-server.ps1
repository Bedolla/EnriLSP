...
<#
.SYNOPSIS
    EnriLSP - Vue Language Server (Volar) installer
.DESCRIPTION
    Checks for vue-language-server installation and auto-installs Node.js runtime if missing.
...
class VueLanguageServerInstaller {
  ...
  VueLanguageServerInstaller() {
    $this.EnvManager = [EnvironmentManager]::new("vue-language-server")
  ...
  [bool] InstallLsp() {
    $this.EnvManager.WriteInfo("Installing @vue/language-server...")
    ...
    $this.AddLspToPath()
    $this.PkgInstaller.InstallWithNpm("@vue/language-server")

    if ($this.IsLspInstalled()) {
      $this.EnvManager.WriteSuccess("vue-language-server installed successfully")
      return $true
    }
  ...
  [int] Run() {
    if ($this.IsLspInstalled()) {
      $this.AddLspToPath()
      $this.EnvManager.WriteSuccess("vue-language-server is already installed")
      return 0
    }
...
[VueLanguageServerInstaller] $installer = [VueLanguageServerInstaller]::new()
exit $installer.Run()

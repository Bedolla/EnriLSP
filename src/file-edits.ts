...
import { uriToPath } from "./lsp/utils.js";
...
function offsetAt(lineOffsets: number[], position: { line: number; character: number }, text: string): number {
...
function applyTextEditsToContent(content: string, edits: LspTextEdit[]): string {
  ...
  const normalized = edits
    .map((edit) => {
      const start = offsetAt(lineOffsets, edit.range.start, content);
      const end = offsetAt(lineOffsets, edit.range.end, content);
...
export async function applyWorkspaceEditToDisk(
  ...
  try {
    for (const [uri, edits] of Object.entries(edit.changes)) {
      const filePath = resolve(uriToPath(uri));
      ...
      const updated = applyTextEditsToContent(original, edits ?? []);

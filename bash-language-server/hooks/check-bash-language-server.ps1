...
<#
.SYNOPSIS
    EnriLSP - Bash Language Server installer
.DESCRIPTION
    Checks for bash-language-server installation and auto-installs Node.js runtime if missing.
...
class BashLanguageServerInstaller {
  ...
  BashLanguageServerInstaller() {
    $this.EnvManager = [EnvironmentManager]::new("bash-language-server")
  ...
  [bool] InstallLsp() {
    $this.EnvManager.WriteInfo("Installing bash-language-server...")
    ...
    $this.AddLspToPath()
    $this.PkgInstaller.InstallWithNpm("bash-language-server")

    if ($this.IsLspInstalled()) {
      $this.EnvManager.WriteSuccess("bash-language-server installed successfully")
      return $true
    }

    $this.EnvManager.WriteError("Failed to install. Please run manually:")
    $this.EnvManager.WriteError("  npm install -g bash-language-server")
    return $false
  }
  ...
  [int] Run() {
    if ($this.IsLspInstalled()) {
      $this.AddLspToPath()
      $this.EnvManager.WriteSuccess("bash-language-server is already installed")
      return 0
    }
...
[BashLanguageServerInstaller] $installer = [BashLanguageServerInstaller]::new()
exit $installer.Run()

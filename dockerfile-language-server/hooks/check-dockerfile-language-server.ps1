...
<#
.SYNOPSIS
    EnriLSP - Dockerfile Language Server installer
.DESCRIPTION
    Checks for dockerfile-language-server installation and auto-installs Node.js runtime if missing.
...
class DockerfileLanguageServerInstaller {
  ...
  DockerfileLanguageServerInstaller() {
    $this.EnvManager = [EnvironmentManager]::new("dockerfile-language-server")
  ...
  [bool] InstallLsp() {
    $this.EnvManager.WriteInfo("Installing dockerfile-language-server-nodejs...")
    ...
    $this.AddLspToPath()
    $this.PkgInstaller.InstallWithNpm("dockerfile-language-server-nodejs")

    if ($this.IsLspInstalled()) {
      $this.EnvManager.WriteSuccess("dockerfile-language-server installed successfully")
      return $true
    }

    $this.EnvManager.WriteError("Failed to install. Please run manually:")
    $this.EnvManager.WriteError("  npm install -g dockerfile-language-server-nodejs")
    return $false
  }
  ...
  [int] Run() {
    if ($this.IsLspInstalled()) {
      $this.AddLspToPath()
      $this.EnvManager.WriteSuccess("dockerfile-language-server is already installed")
      return 0
    }
...
[DockerfileLanguageServerInstaller] $installer = [DockerfileLanguageServerInstaller]::new()
exit $installer.Run()

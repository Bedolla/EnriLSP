...
<#
.SYNOPSIS
    EnriLSP - Texlab (LaTeX Language Server) installer
.DESCRIPTION
    Checks for texlab installation and auto-installs from GitHub releases.
...
class TexlabInstaller {
  ...
  hidden [string[]] $LspKnownPaths = @(
    ...
    "$env:LOCALAPPDATA\EnriLSP\bin\texlab.exe",
  ...
  hidden [string] $GitHubReleaseApi = "https://api.github.com/repos/latex-lsp/texlab/releases/latest"
  ...
  TexlabInstaller() {
    $this.EnvManager = [EnvironmentManager]::new("texlab")
  ...
  [bool] EnsureStableInstall() {
    try {
      [string] $target = Join-Path $this.InstallDir "texlab.exe"
      if (Test-Path $target -PathType Leaf) {
        return $true
      }

      # Backward-compat: if legacy install exists, migrate it to the centralized bin
      [string] $legacyExe = "$env:LOCALAPPDATA\texlab\texlab.exe"
      ...
      if ([string]::IsNullOrWhiteSpace($found)) {
        $this.EnvManager.WriteError("texlab executable not found after install")
        return $false
      }
      ...
      $this.EnvManager.WriteError("Failed to ensure stable texlab install: $($_.Exception.Message)")
      return $false
    }
  }
  ...
  [string] GetLatestReleaseUrl() {
    try {
      ...
      foreach ($asset in $release.assets) {
        if ($asset.name -match "texlab.*x86_64-windows\.zip$") {
  ...
  [bool] InstallFromGitHub() {
    ...
    try {
      ...
      [string] $zipPath = "$env:TEMP\texlab.zip"
      $this.EnvManager.WriteInfo("Downloading texlab...")
      ...
      # Some zips include a folder; if texlab.exe isn't at root, try to locate it and move it
      [string] $expected = Join-Path $this.InstallDir "texlab.exe"
      if (-not (Test-Path $expected -PathType Leaf)) {
        $foundExe = Get-ChildItem -Path $this.InstallDir -Recurse -Filter "texlab.exe" -File -ErrorAction SilentlyContinue | Select-Object -First 1
        if ($null -ne $foundExe) {
  ...
  [bool] InstallLsp() {
    $this.EnvManager.WriteInfo("Installing texlab...")
    ...
    if ($wingetResult.Success) {
      ...
      if ($this.IsLspInstalled()) {
        $this.AddLspToPath()
        $this.EnvManager.WriteSuccess("texlab installed via winget")
        return $true
      }
    }
    ...
    if ($this.InstallFromGitHub() -and $this.IsLspInstalled()) {
      $this.EnvManager.WriteSuccess("texlab installed from GitHub")
      return $true
    }

    $this.EnvManager.WriteError("Failed to install texlab")
    return $false
  }
  ...
  [int] Run() {
    ...
    if ($this.IsLspInstalled()) {
      ...
      $this.EnvManager.WriteSuccess("texlab is already installed")
      return 0
    }
...
[TexlabInstaller] $installer = [TexlabInstaller]::new()
exit $installer.Run()

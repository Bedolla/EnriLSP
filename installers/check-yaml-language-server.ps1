...
<#
.SYNOPSIS
    EnriLSP - YAML Language Server installer
.DESCRIPTION
    Checks for yaml-language-server installation and auto-installs Node.js runtime if missing.
...
class YamlLanguageServerInstaller {
  hidden [EnvironmentManager] $EnvManager
  ...
  YamlLanguageServerInstaller() {
    $this.EnvManager = [EnvironmentManager]::new("yaml-language-server")
  ...
  [bool] InstallLsp() {
    $this.EnvManager.WriteInfo("Installing yaml-language-server...")
    ...
    $this.AddLspToPath()
    $this.PkgInstaller.InstallWithNpm("yaml-language-server")

    if ($this.IsLspInstalled()) {
      $this.EnvManager.WriteSuccess("yaml-language-server installed successfully")
      return $true
    }

    $this.EnvManager.WriteError("Failed to install. Please run manually:")
    $this.EnvManager.WriteError("  npm install -g yaml-language-server")
    return $false
  }
  ...
  [int] Run() {
    if ($this.IsLspInstalled()) {
      $this.AddLspToPath()
      $this.EnvManager.WriteSuccess("yaml-language-server is already installed")
      return 0
    }
...
[YamlLanguageServerInstaller] $installer = [YamlLanguageServerInstaller]::new()
exit $installer.Run()
